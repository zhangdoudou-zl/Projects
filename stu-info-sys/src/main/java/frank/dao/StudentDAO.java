...
public class StudentDAO {
    ...
    public static void update(Student s) {
        ...
        try {
            ...
            c = DBUtil.getConnection();
            //复制粘贴进来的代码：ctrl+f替换所有换行符，在需要空格的地方加上空格或tab缩进，设置占位符
            String sql = "update student set student_name=?, student_no=?, id_card=?, student_email=?, classes_id=? where id=?";//关联班级表和学生表，查询需要的数据（学生的数据和班级的数据）
            //2.创建操作命令对象
            ps = c.prepareStatement(sql);
            ps.setString(1, s.getStudentName());
            ps.setString(2, s.getStudentNo());
    ...
    public static void delete(String[] ids) {
        ...
        try {
            ...
            c = DBUtil.getConnection();
            //复制粘贴进来的代码：ctrl+f替换所有换行符，在需要空格的地方加上空格或tab缩进，设置占位符
            StringBuilder sql = new StringBuilder("delete from student where id in (");//关联班级表和学生表，查询需要的数据（学生的数据和班级的数据）
